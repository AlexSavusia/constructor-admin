...
export default function InputDate({label, value = "", onChange, disabled}: Props) {
    ...
    return (
        <div className="input-date-container" ref={rootRef}>
            <div className="input-date">
                <input
                    type="text" className="input-date-input" value={inputValue} onChange={handleInputChange} placeholder=" " disabled={disabled}
                />

                <label className="input-date-label">{label}</label>

                <button type="button" className="input-date-button" onClick={() => setOpen((prev) => !prev)} disabled={disabled} aria-label="Открыть календарь"
                >
            ...
            {open && (
                <div className="input-date-popper">
                    <DayPicker mode="single" selected={selected} onSelect={handleDaySelect} locale={ru} captionLayout="dropdown" fromYear={1900} toYear={2100}/>
                </div>
            )}
        </div>
    );
