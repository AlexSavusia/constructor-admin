import {
    type ObjPath, objPathFromString, objPathToString,
} from "./Programs/editor/EditorContext.tsx";
...
const getBooleanExpressionDependentPaths = (expr: BooleanExpression): ObjPath[] => {
    ...
    switch (expr.type) {
        ...
        case "ne":
            return [
                getValueExpressionDependentPaths(expr.left),
                getValueExpressionDependentPaths(expr.right)
            ]
        case "isEmpty":
...
function evalActionExpression(
    ...
    onSetFieldProperty: (fieldKey: Key, propertyKey: 'enabled' | 'visibility' | 'required' | 'value', value: boolean) => Partial<FormContextValue>
    ...
    switch (action.type) {
        ...
        case "setFieldProperty": {
            return onSetFieldProperty(selfFieldKey, action.property as 'enabled' | 'visibility' | 'required' | 'value', action.value)
        }
        case "noop":
        default:
...
const createFormContext = (initialState: FormDefinition): StoreApi<FormContextValue> => {
    ...
    const fieldsDeps = (): Record<string, ObjPath[]> => {
        return Object.keys(fieldsValues).filter(key=>{
    ...
    return createStore<FormContextValue>((set, get) => ({
        ...
        setFieldEnabled: (key, value) => set((state) => ({
            fieldEnabled: {
                ...state.fieldEnabled,
                [key]: value,
            }
        })),
        setFieldRequired: (key, value) => set((state) => ({
            fieldRequired: {
                ...state.fieldRequired,
                [key]: value,
            }
        ...
        evalAction: (action, fieldKey) => set((state) => {
            return evalActionExpression(action, state, fieldKey,
                (fieldKey, error) => {
                    return {
                        fieldsErrors: {
                            ...state.fieldsErrors,
        ...
        clearFieldErrors: (fieldKey: Key) => set((state) => {
            // eslint-disable-next-line @typescript-eslint/no-unused-vars
...
function InputFieldRenderer({field, path}: FieldRendererProps) {
    ...
    const depPaths = useFormContext(
        useShallow(s => s.fieldsDeps[pathString] ?? [])
    )
    const deps = useFormContext(
        useShallow(state => depPaths.map(depPath => state.fieldsValues[objPathToString(depPath)]))
    )
...
// debugger
    switch (field.control) {
        case "input": {
            switch (field.settingsValues['fieldType'] as 'description' | 'input' | 'textarea' | 'checkbox' | 'radio' | 'switch' | 'date' | 'select' | 'file'){
                case "input": {
                    return (
                        <div>
                        ...
                        {fields.map((item) => (
                            <div key={item.key}>
                                {item.fieldType == "input" ? (
                                    <InputFieldRenderer key={item.key} field={item} path={["steps", currentStepKey, "fields", item.key]}/>
                                ) : (
                                    <OutputFieldRenderer key={item.key} field={item} path={["steps", currentStepKey, "fields", item.key]}/>
                                )}
                            </div>
                        ))}
                    </ReactGridLayout>
